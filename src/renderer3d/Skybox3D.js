...
const SKY_NEAR   = 0x0f1c2a;
const SKY_FAR    = 0x060c14;
const BUILD_FAR  = 0x0b1520;
const BUILD_NEAR = 0x0f1d2c;
const WIN_COLOR  = 0xffee88;
const MOON_COLOR = 0xfff5cc;

// Aurora colour: green-teal shift over time.
const AURORA_COLOR_A = new THREE.Color(0x00ff88);
const AURORA_COLOR_B = new THREE.Color(0x0088ff);
...
export class Skybox3D {
  constructor(scene) {
    ...
    // These refs are set by the _build* methods below.
    this._moonMesh      = null;
    this._moonGlowMat   = null;
    this._starColors    = null;   // Float32Array for twinkle
    this._starPoints    = null;   // THREE.Points
    this._starPhases    = null;   // Float32Array per star
    this._starSpeeds    = null;   // Float32Array per star
    this._auroraGeo     = null;
    this._auroraMat     = null;
    // After rotateX the ribbon is flat (all Y=0); wave simply displaces from 0.
  ...
  // ── Sky gradient ──────────────────────────────────────────────────────────────
  _buildSky() {
    const geo = new THREE.PlaneGeometry(80, 40, 1, 3);
    const col = new THREE.Color();
    const pos = geo.attributes.position;
    const colours = [];
    for (let i = 0; i < pos.count; i++) {
      const t = (pos.getY(i) + 20) / 40;
      col.lerpColors(new THREE.Color(SKY_NEAR), new THREE.Color(SKY_FAR), t);
  ...
  _buildStars() {
    ...
    for (let i = 0; i < COUNT; i++) {
      // Distribute on upper dome (hemisphere facing camera).
      const theta = Math.random() * Math.PI * 2;
      const phi   = Math.random() * Math.PI * 0.52;   // upper 52% of sphere
      const r     = 44 + Math.random() * 4;
      positions[i * 3]     = r * Math.sin(phi) * Math.cos(theta);
      positions[i * 3 + 1] = Math.abs(r * Math.cos(phi)) + 4;   // above Y=0
      positions[i * 3 + 2] = r * Math.sin(phi) * Math.sin(theta) - 22;

      const base = 0.75 + Math.random() * 0.25;
      colors[i * 3]     = base;
      colors[i * 3 + 1] = base;
      colors[i * 3 + 2] = 0.90 + Math.random() * 0.10;   // slight blue tint
  ...
  // ── Aurora ribbon ─────────────────────────────────────────────────────────────
  _buildAurora() {
    // Wide plane with many X subdivisions so we can wave it in update().
    const SEG_X = 60;
    const SEG_Y = 2;
    const geo = new THREE.PlaneGeometry(56, 5, SEG_X, SEG_Y);
    geo.rotateX(-Math.PI / 2);   // lay flat; after rotation all Y=0 (wave from zero)

    this._auroraGeo = geo;
    this._auroraMat = new THREE.MeshBasicMaterial({
      color:       AURORA_COLOR_A,
      transparent: true,
      opacity:     0.0,
    ...
    for (const c of configs) {
      ...
      for (let i = 0; i < winCount; i++) {
        ...
        const winMat = new THREE.MeshBasicMaterial({
          color: WIN_COLOR, transparent: true, opacity: 0.4,
        });
  ...
  update(dt) {
    ...
    const amplitude = 0.4 + comboAmp * 2.0;    // grows with combo
    const speed     = 1.2 + comboAmp * 1.5;
    const targetOpacity = 0.08 + comboAmp * 0.30;

    // Lerp opacity toward target.
    this._auroraMat.opacity += (targetOpacity - this._auroraMat.opacity) * Math.min(1, dt * 2);

    // Colour shifts between green-teal.
    const colLerp = 0.5 + 0.5 * Math.sin(t * 0.3);
    const auroraCol = new THREE.Color();
    auroraCol.lerpColors(AURORA_COLOR_A, AURORA_COLOR_B, colLerp);
