...
import { benchY, benchSlotH } from '../renderer/BenchRenderer.js';
...
// solution is benching (2026-07-16 playtest).
//
// FUNCTION, not a module-level const (2026-07-24 geometry-liveness sweep):
// bombSlotScreenY/PX_PER_WU/BOMB_R and benchY() are all lane-count-keyed and
// mutable since 63092fd. Frozen at import (band-540 values ≈692), it sat ABOVE
// the live boundary (≈705) on 3-lane levels, so drops on row-2's lower half
// misrouted to the bench — the exact inverse of the bug this boundary exists
// to prevent.
export function queueBenchBoundaryY() {
  return ((bombSlotScreenY(2) + PX_PER_WU * BOMB_R) + (benchY() - 4)) / 2;
}
...
export class DragDrop {
  ...
  _hitTestQueueSlot(x, y) {
    ...
    // circular radius must not shadow the tray (see queueBenchBoundaryY).
    if (y >= queueBenchBoundaryY()) return null;
